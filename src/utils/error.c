...
void handle_error(int exit_code, char *type)
{
	if(!ft_strcmp(type, WRONG_EXTENTION))
		...
		ft_putstr_fd("Please make sure that you set correct way for player to get all coins and exit from game!\n", 2);
	else ft_putstr_fd("Something went wrong. Please check map configuration.\n", 2);
	if(exit_code)
		exit(exit_code);
}